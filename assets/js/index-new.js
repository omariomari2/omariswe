...
function initVisualFilter() {
  ...
  $(document).ready(function(){
    ...
    if ($('#work').length) {
      ...
      $('.design-btn').addClass('active').removeClass('not-active');
      // Set subtitles
      $('.grid-sub-title-certifications').hide();
      $('.grid-sub-title-development').hide();
      $('.grid-sub-title-design').show();
      // Show only design items across all listing UIs, including table rows
      $('.mouse-pos-list-image li, .mouse-pos-list-image-wrap li, .work-tiles li, .work-items li').removeClass('visible');
